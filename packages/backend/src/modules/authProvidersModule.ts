      ...
      return providers.oauth2Proxy.create({
        signIn: {
          async resolver({ result }, ctx) {
            const name = process.env.OAUTH_USER_HEADER ? result.getHeader(process.env.OAUTH_USER_HEADER) : result.getHeader('x-forwarded-preferred-username') || result.getHeader('x-forwarded-user');
            if (!name) {
